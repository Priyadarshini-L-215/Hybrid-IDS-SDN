...
# Force Keras backend before any ML imports
...
def main():
    ...
    if not Path(dataset_path).exists():
        print(f"Dataset not found at {dataset_path}, falling back to data/dataset.csv")
        dataset_path = "data/dataset.csv"
        if not Path(dataset_path).exists():
            print(f"Fallback dataset not found at {dataset_path}")
            return

    print(f"Loading dataset: {dataset_path}...")
    df = pd.read_csv(dataset_path)
    
    if "UNSW_NB15" in dataset_path:
        # Mapping for raw UNSW-NB15 dataset to model features
        mapping = {
            'dur': 'flow_duration',
            'spkts': 'spkts',
            'dpkts': 'dpkts',
